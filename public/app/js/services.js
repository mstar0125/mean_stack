...
angular.module('xenon.services', []).
	service('$menuItems', function($rootScope)
	{
		...
		{
			if($rootScope.game == 'EU') {
				var dashboard    = this.addItem('EU Games', 			'/app/home', 			'');
				var dashboard1	 = this.addItem('Prize Pools',  		'/app/prize',           '');				
			}
			else {
				var dashboard    = this.addItem('US Games', 			'/app/homeUS', 			'');
				var dashboard1	 = this.addItem('Prize Pools',  		'/app/prizeUS',         '');
			}
			var dashboard2	 = this.addItem('Age Verification',  	'/app/verification',    '');
			var dashboard3	 = this.addItem('Withdraw Request',  	'/app/withdraw',        '');
			var dashboard4 	 = this.addItem('Users',				'/app/users',			'');
			var dashboard5 	 = this.addItem('FacebookID',			'/app/fbList',			'');
			return this;
		}
	...
	.service('authentication', function($http, $window, $rootScope)
	{
	    ...
	    var register = function(user) {
		  return $http.post($rootScope.api_path + 'register', user).success(function(data){
		    saveToken(data.token);
		  });
		};

		var login = function(user) {
		  return $http.post($rootScope.api_path + 'login', user).success(function(data) {
		    saveToken(data.token);
		  });
		};
